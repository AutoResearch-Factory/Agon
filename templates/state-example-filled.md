...
| 投毒探针 | 一段看起来像真的但结论是假的文本。三种：DUP（重复记录）、FORMAT（表格）、SINK（灌水） |
| RouteTrace | 压缩时保存的"谁被选中了"的记录——每层分别选了哪些 token |
| Replay | 保存完整 KV 备份 → 事后任意修改"选中谁" → 重新评分。整个机制实验的基石 |
...
|------|---------|------|------|------|---------|
| R0 | KVPress 能被 trace/replay 吗？（门禁） | Qwen2.5-7B, Qwen3-8B | SciFact 32条 | ✅ | 4/4 通过 |
...
- **R0**：Qwen2.5-7B 4/4 PASS，Qwen3-8B 3/4 PASS（snapkv|0.5 有 3 条边缘情况，PASS-with-asterisk）。门禁打开：KVPress 能被 trace 和 replay。
- **R1/R2/R4** 的第一段均正常分段退出，checkpoint 已保存。

### 4.2 关键警告

1. **random +23.6%**：论文机制主张的最大威胁。需决策是否加对比实验（注意力压缩 vs 随机压缩）。
2. **EA|0.5 clean flip 21pp**：ExpectedAttention 在 50% 压缩比下，无毒的良性文本翻转率达 21%（阈值 ≤2pp）。该 arm 已移出核心 claims。

### 4.3 Claims 速查

| claim_id | 要证明的事 | evidence_refs | entailment | 强度 |
|----------|-----------|---------------|------------|------|
| C1 | KVPress 能被我们偷看和操作（门禁） | commit:abc1234; run:results/stage0_hook_audit/manifest.json; result:results/stage0_hook_audit/summary.json#pass_rate; audit:audits/audit_iter8_example.md | SUPPORTS | 够 |
...
3. **门禁实验**：KVPress 能被 trace/replay 吗？（R0）
...
- Success criterion: 至少一个 (probe × position × policy × ratio) cell 的 paired Δ 有 |Δ| ≥ +0.05，95% CI 排除 0
- Failure interpretation: 全 null → 退到 R2/R3；全负 → 跨代"压缩即去噪"强化
...
- remote_dir: `/srv/agon/example-workspaces/dsv4x-compress-poison`
- Success criterion: 至少一个 ratio 下 paired Δ 有 |Δ| ≥ +0.05，CI 排除 0
- Failure interpretation: 全 ratio 倒置 → "压缩即去噪" ratio 单调，replay 决定是否发表
...
- remote_dir: `/srv/agon/example-workspaces/dsv4x-compress-poison`
- Success criterion: ≥80 clean-correct items，且 POISON_FULL − LENGTH_ONLY_FULL Δ ≥ +0.10 或显式 null
- Failure interpretation: null → 升级到 scbench_choice_eng / scbench_repoqa
...
- 验证项: 可见性（token indices 可见）、可 replay（同状态 swap IDs）、mask-OOD 恒等、benign-twin ≤2pp
- 输出: `results/stage0_kvpress_audit_qwen7b_v2/stage0_summary.json`, `results/stage0_audit_qwen3_8b_v2/stage0_summary.json`
- 结果: Qwen2.5-7B 4/4 PASS（clean acc 0.94/0.88）；Qwen3-8B 3/4 PASS（clean acc 0.9375），snapkv|0.5 有 3 条 drop_poison 边缘情况
