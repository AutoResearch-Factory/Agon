---
phase: needs_scientist                # dispatcher 状态机: needs_scientist | needs_screener | coding_and_running | needs_auditor | needs_reviewer | done
...
<!-- 只有需要反复使用, 且领域中没有稳定沿用术语的概念, 才能新造术语. 所有新造术语必须先在此表定义, 再在后文使用; 未列入此表的自造名称, 词组或缩写不得出现在 STATE.md 其他位置. -->
...
## 自检清单 (提交前必做, 14/14 通过才能 commit)

1. 闭眼 30 秒, 一句话说出项目在研究什么, 核心发现是什么. 说不出来 → §1 没写好
2. §1 中每个术语首次出现时, 是否用一句大白话解释? 没有 → 补; 无法准确解释 → 删除或改用稳定术语
3. §1.1 超过 15 行? → 你在写综述, 删
4. 任选 §2-§6 连续 3 句话, 实习生读到会问"这跟我有什么关系"? 会 → 重写
5. 任选一段删掉, 实习生仍能理解项目? 能 → 删掉
6. `wc -l STATE.md` > 400? → 删到 ≤400, 不许挪到 A 段绕过
7. 已发表论文已有稳定术语, STATE.md 却另造名称或罕见词组? 有 → 改用论文术语
8. 有没有给已有概念或普通现象另起一个貌似正式的名字? 有 → 删除名称, 直接描述
9. 同一概念是否前后换名, 或给项目已有名称另造别名? 有 → 统一名称
10. 有没有自造缩写, 或使用未展开, 未定义的非通行缩写? 有 → 写全称或定义
11. "本项目自造术语表" 中的每个词是否需要反复使用, 领域中没有稳定术语, 且已有定义? 任一项否 → 删除, 改用论文术语或直接描述
12. STATE.md 是否出现不在 "本项目自造术语表" 中的自造名称, 罕见词组, 缩写, 拼接词, 符号串或密集简写? 有 → 满足条件后列入表并定义, 否则改用论文术语或直接描述
13. 本轮 collected 结果是否逐项与本轮其他 runs, 历史 runs 和已发表论文比较, 并主动看图? 没有 -> 先做异常感知
14. §4.2 中每个未定位异常是否列出按可能性排序的原因和能区分它们的诊断实验或分析? 没有 -> 补
...
- Expected outputs: 期待看到的实验现象及理由
- Artifacts: `results/<run-name>/manifest.json`, 原始结果, logs, 分析图, remote-only assets if any
- Priority: MUST-RUN / NICE-TO-HAVE
- Compute cost: GPU-hours 估算
- Server: primary + fallback
- remote_dir:
- Anomaly localization (仅定位实验): 对应 §4.2 中哪个异常, 如何区分候选原因
- Risk: OOM / 数据 / 兼容性
