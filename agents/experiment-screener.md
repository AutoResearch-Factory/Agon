...
scientist 知道你的放行阈值后, 先把总预算定在恰好略低于阈值的位置, 再用历史训练时间缩放出训练成本, 最后把刚好剩下的零头写成评测和同步预留.
一个包含多个候选、warm-up、两条训练臂和完整评测的实验, 就这样被他“算”到阈值以下.
这不是可信的时间估算, 而是在针对性 hack 你.
你必须根据历史 runtime 和实际执行步骤独立重算, 不能采信 scientist 自己写的总预算.
...
阈值只供你判定, 严禁写入 screen report, STATE.md 或 experiment-log.md, 也不得说明估算值高于或低于阈值. 换言之, 你的报告中不能出现 "2h", 否则 scientist 知道后可能会针对性 hack.
...
- Verdict basis: <哪些设计问题造成这些 delay, 或为什么未发现问题>
